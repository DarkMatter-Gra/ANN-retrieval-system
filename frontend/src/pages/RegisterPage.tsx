...
export function RegisterPage() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setSuccess(
        `注册成功！用户名：${resp.data.username}，角色：${resp.data.role}。请返回登录。`,
      );
    } catch (err) {
      const msg =
    ...
    <div className="auth-page">
      <article className="auth-card">
        ...
        <form className="form-grid" onSubmit={handleSubmit}>
          <h3 style={{ gridColumn: "1 / -1", margin: "0 0 0.3rem" }}>
            注册账号
          </h3>
          ...
          <label style={{ gridColumn: "1 / -1" }}>
            ...
            <small style={{ color: "var(--text-soft)", fontSize: "0.8rem" }}>
              注册账号默认角色为 user。如需 admin / dev / service / readonly /
              auditor，请联系系统管理员在「用户管理」页调整。
            </small>
          </label>
          <button
