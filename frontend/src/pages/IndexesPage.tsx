...
export function IndexesPage() {
  ...
  const [selectedIndexId, setSelectedIndexId] = useState<number | null>(
    () => Number(localStorage.getItem(IDX_KEY) || 0) || null,
  );
  const [indexDetail, setIndexDetail] = useState<Record<
    string,
    unknown
  > | null>(null);
  ...
  const loadDetail = useCallback(
    async (id: number) => {
      try {
        const resp = await apiCall<Record<string, unknown>>({
          baseUrl,
          token,
          path: `/indexes/${id}`,
        });
    ...
    try {
      if (action === "load") {
        await apiCall({
          baseUrl,
          token,
          path: `/indexes/${selectedIndexId}/load`,
          method: "POST",
        });
        ...
        await apiCall({
          baseUrl,
          token,
          path: `/indexes/${selectedIndexId}/publish`,
          method: "POST",
          body: { audit_comment: auditComment },
        });
      } else {
        await apiCall({
          baseUrl,
          token,
          path: `/indexes/${selectedIndexId}/rollback`,
          method: "POST",
          body: { target_version: Number(rollbackVersion || 0) },
        });
      }
    ...
    <div className="page-container">
      ...
      <article className="panel">
        <div className="panel-head panel-head-row">
          <div>
            <h3>索引列表</h3>
            <p>当前数据集 ID：{selectedDatasetId ?? "-"}</p>
          </div>
        ...
        <div className="toolbar">
          ...
          <label className="toolbar-field small">
            ...
            <input
              ...
              value={pageSize}
              onChange={(e) =>
                setPageSize(clamp(Number(e.target.value) || 20, 1, 100))
              }
            />
          </label>
          <button
              ...
              {indexes.length ? (
                indexes.map((idx) => (
                  <tr
                    key={idx.index_id}
                    className={
                      idx.index_id === selectedIndexId ? "active-row" : ""
                    }
                  >
                    <td>
                      <strong>{idx.index_id}</strong>
                    </td>
                    ...
                    <td>
                      召回：{idx.recall_score ?? idx.recall ?? "-"} / 内存：
                      {formatBytes(idx.memory_cost_mb ?? idx.memory_usage ?? 0)}
                    </td>
                    <td>
                      <div className="row-actions">
                        <button
                          className="btn btn-secondary"
                          type="button"
                          onClick={() => setSelectedIndexId(idx.index_id)}
                        >
                          选中
                        </button>
                        <button
                          className="btn btn-secondary"
                          type="button"
                          onClick={() => void loadDetail(idx.index_id)}
                        >
                          详情
                        </button>
                        <button
        ...
        <article className="panel">
          ...
          <div className="detail-box" style={{ marginTop: "1rem" }}>
            {indexDetail ? (
              ...
              >
                {JSON.stringify(indexDetail, null, 2)}
              </pre>
            ) : (
