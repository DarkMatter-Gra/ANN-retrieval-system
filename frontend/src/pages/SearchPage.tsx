...
export function SearchPage() {
  ...
  async function handleSearch(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      const resp = await apiCall<SearchResponse>({ baseUrl, token, path: '/search', method: 'POST', body: payload });
  ...
  async function loadHighlights() {
    if (!highlightQueryId) { showToast('请输入 query_id', 'error'); return; }
    try {
      const resp = await apiCall<{ query_id: string; neighbors: { cell_id: string; point: number[] }[] }>({
        baseUrl, token, path: `/visualizations/highlights/${highlightQueryId}`,
      });
    ...
    <div className="page-container">
      ...
      <div className="detail-grid">
        <article className="panel">
          <div className="panel-head"><h3>检索参数</h3></div>
          <form className="form-grid" onSubmit={handleSearch}>
            ...
            <label className="full">
              ...
              <textarea name="filters" rows={3} defaultValue="{}" spellCheck={false} />
            </label>
          ...
          {results && (
            <div className="summary-box" style={{ marginTop: '1rem' }}>
              <div><strong>query_id：</strong>{results.query_id ?? '-'}</div>
              <div><strong>latency_ms：</strong>{results.latency_ms ?? '-'}</div>
              <div><strong>recall_estimate：</strong>{results.recall_estimate ?? '-'}</div>
              <div><strong>结果数：</strong>{searchList.length}</div>
            </div>
          )}
        </article>
        ...
        <div style={{ display: 'grid', gap: '1.2rem' }}>
          <article className="panel">
            <div className="panel-head"><h3>检索结果</h3></div>
            <div className="result-list">
              {searchList.length ? searchList.map((item) => (
                <article key={`${item.rank}-${item.cell_id}`} className="result-card">
                  <header>
                    <div>
                      <h5>#{item.rank} {item.cell_id}</h5>
                      <p>{item.cell_type ?? '-'} | {item.organ ?? '-'} | {item.sample_id ?? '-'}</p>
                    </div>
          ...
          <article className="panel">
            <div className="panel-head"><h3>检索高亮</h3></div>
            <div className="detail-box">
              <div style={{ display: 'flex', gap: '0.6rem', marginBottom: '0.8rem' }}>
                <input
                  ...
                  style={{ flex: 1 }}
                />
                <button className="btn btn-secondary" type="button" onClick={() => void loadHighlights()}>加载高亮</button>
              </div>
              {highlights ? (
                <>
                  <div><strong>query_id：</strong>{highlights.query_id ?? '-'}</div>
                  <div style={{ marginTop: '0.6rem' }}>
                    <strong>邻域点（{highlights.neighbors?.length ?? 0} 个）</strong>
                    {highlights.neighbors?.slice(0, 5).map((n) => (
                      <div key={n.cell_id} className="muted" style={{ fontSize: '0.85rem' }}>{n.cell_id}</div>
                    ))}
                    {(highlights.neighbors?.length ?? 0) > 5 && <div className="muted">…更多</div>}
                  </div>
                </>
