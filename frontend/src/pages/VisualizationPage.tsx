...
function computeView(embedding: EmbeddingResponse, colorBy: string) {
  ...
  const numericKeys = Object.keys(sample).filter((k) => k !== 'cell_id' && typeof sample[k] === 'number');
  ...
  const W = 1000; const H = 640; const P = 40;
  const scaleX = (x: number) => P + ((x - xMin) / (xMax - xMin || 1)) * (W - P * 2);
  const scaleY = (y: number) => H - P - ((y - yMin) / (yMax - yMin || 1)) * (H - P * 2);

  const legendValues = colorBy ? Array.from(new Set(values.map((v) => v.colorValue))) : [];
...
export function VisualizationPage() {
  ...
  const [datasetId, setDatasetId] = useState(() => Number(localStorage.getItem(DS_KEY) || 0) || null as number | null);
  ...
  async function loadEmbedding() {
    if (!datasetId) { showToast('请输入数据集 ID', 'error'); return; }
    try {
      const resp = await apiCall<EmbeddingResponse>({
        baseUrl, token, path: `/visualizations/${datasetId}/embedding`,
        query: { method, page: 1, page_size: pageSize, color_by: colorBy || undefined },
      });
    ...
    <div className="page-container">
      ...
      <article className="panel">
        <div className="toolbar">
          ...
          <label className="toolbar-field small">
            <span>降维方法</span>
            <select value={method} onChange={(e) => setMethod(e.target.value as 'umap' | 'tsne')}>
          ...
          <label className="toolbar-field small">
            ...
            <input
              ...
              value={pageSize}
              onChange={(e) => setPageSize(clamp(Number(e.target.value) || 2000, 1, 20000))}
            />
          </label>
          <button className="btn btn-primary" type="button" onClick={() => void loadEmbedding()}>加载点位</button>
        </div>

        <div className="viz-grid">
          <div className="viz-canvas-wrap">
            <svg id="embedding-canvas" viewBox="0 0 1000 640" preserveAspectRatio="none">
              <defs>
              ...
              <line x1="40" y1="600" x2="960" y2="600" stroke="url(#axisGlow)" strokeWidth="1.2" />
              <line x1="40" y1="40" x2="40" y2="600" stroke="url(#axisGlow)" strokeWidth="1.2" />
              {view?.values.map((v) => {
                const color = view.legendValues.length ? (view.colorMap.get(v.colorValue) ?? '#69e2c3') : '#69e2c3';
                return (
                  <circle
                    key={v.cell_id}
                    className="viz-point"
          ...
          <aside className="viz-side">
            <div className="summary-box">
              {embedding ? (
                <>
                  <div><strong>方法：</strong>{embedding.method ?? method}</div>
                  <div><strong>总点数：</strong>{embedding.total ?? 0}</div>
                  <div><strong>当前渲染：</strong>{view?.values.length ?? 0}</div>
                  <div><strong>颜色字段：</strong>{colorBy || 'none'}</div>
                </>
              ) : <span className="empty-state">加载后显示统计信息</span>}
            </div>
            <div className="legend-box">
              {view?.legendValues.length ? view.legendValues.map((v, i) => (
                <div key={v} className="legend-item">
                  <span className="legend-dot" style={{ background: paletteForIndex(i) }} />
                  {v}
                </div>
