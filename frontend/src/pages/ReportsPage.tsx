  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      const resp = await apiCall<ReportResult>({
        baseUrl, token, path: '/reports/diagnostic', method: 'POST',
        body: {
    ...
    <div className="page-container">
      ...
      <div className="detail-grid">
        <article className="panel">
          <div className="panel-head"><h3>生成报告</h3></div>
          <form className="form-grid" onSubmit={handleSubmit}>
            <label>
              <span>数据集 ID（可选）</span>
              <input name="dataset_id" type="number" min="1" placeholder="留空则全局诊断" />
            </label>
            <label>
              <span>索引 ID（可选）</span>
              <input name="index_id" type="number" min="1" placeholder="留空则跳过索引" />
            </label>
            <label style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', gridColumn: '1 / -1' }}>
              <input name="include_qc" type="checkbox" defaultChecked style={{ width: 'auto' }} />
              <span>包含 QC 报告</span>
            </label>
            <label style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', gridColumn: '1 / -1' }}>
              <input name="include_performance" type="checkbox" defaultChecked style={{ width: 'auto' }} />
              <span>包含性能报告</span>
            </label>
        ...
        <article className="panel">
          <div className="panel-head"><h3>报告内容</h3></div>
          <div className="detail-box" style={{ minHeight: '300px' }}>
            {report ? (
              <pre style={{ whiteSpace: 'pre-wrap', margin: 0, fontSize: '0.85rem', lineHeight: 1.6 }}>
