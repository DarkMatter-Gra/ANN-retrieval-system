...
export function DatasetsPage() {
  ...
  const loadDetail = useCallback(
    async (id: number) => {
      try {
        const resp = await apiCall<DatasetDetail>({
          baseUrl,
          token,
          path: `/datasets/${id}`,
        });
  ...
  const loadLogs = useCallback(
    async (id: number) => {
      try {
        const resp = await apiCall<DatasetLogs>({
          baseUrl,
          token,
          path: `/datasets/${id}/logs`,
        });
        setLogs(resp.data);
      } catch (err) {
        handleError(err);
  ...
  useEffect(() => {
    const hasPending = datasets.some(
      (d) =>
        !isTerminalStatus(d.qc_status) ||
        !isTerminalStatus(d.preprocess_status),
    );
  ...
  async function handleUpload(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      showToast(
        `上传成功，数据集 ID: ${completeResp.data.dataset_id}`,
        "success",
      );
  ...
  return (
    <div className="page-container">
      ...
      <article className="panel">
        <div className="panel-head panel-head-row">
          <div>
            <h3>数据集列表</h3>
            <p>
              共 {total} 个数据集（当前页 {datasets.length} 条）
            </p>
          </div>
              ...
              {datasets.length ? (
                datasets.map((d) => (
                  ...
                  >
                    ...
                    <td style={{ color: "var(--accent)", fontSize: "0.88rem" }}>
                      {d.file_format ?? "-"}
                    </td>
                    ...
                    <td
                      style={{
                        fontSize: "0.88rem",
                        color: isTerminalStatus(d.qc_status)
                          ? d.qc_status === "failed"
                            ? "var(--danger)"
                            : "var(--accent)"
                          : "var(--text-soft)",
                    ...
                    >
                      {statusLabel(d.qc_status)}
                    </td>
                    <td
                      style={{
                        fontSize: "0.88rem",
                        color: isTerminalStatus(d.preprocess_status)
                          ? d.preprocess_status === "failed"
                            ? "var(--danger)"
                            : "var(--accent)"
                          : "var(--text-soft)",
                      }}
                    >
                      {statusLabel(d.preprocess_status)}
                    </td>
                    <td
                    ...
                    <td>
                      <div className="row-actions">
                        ...
                        <button
                          ...
                          onClick={(ev) => {
                            ev.stopPropagation();
                            void handleDelete(d.dataset_id);
                          }}
                        >
                          删除
                        </button>
        ...
        {total > 0 && (
          <div
            className="inline-actions spacing-top"
        ...
        <div className="detail-grid">
          <article className="panel">
            <div className="panel-head">
              ...
              <p>
                数据集 ID：
                <strong style={{ color: "var(--accent)" }}>{selectedId}</strong>
              </p>
            </div>
      ...
      <article className="panel">
        ...
        <form
          className="form-grid upload-grid"
            ...
            <div className="full" style={{ display: "grid", gap: "0.4rem" }}>
              <div
                style={{
                  height: 8,
                  borderRadius: 4,
                  background: "rgba(156,175,199,0.14)",
                  overflow: "hidden",
                }}
              >
                <div
