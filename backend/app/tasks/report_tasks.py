...
def generate_report_task(self, task_id: str):
    ...
    try:
        ...
        dataset_id = int(payload.get("dataset_id") or task.dataset_id)
        index_id = payload.get("index_id") or task.index_id
        ...
        query_payload = _build_query_payload(db, payload, query_id)
        ...
        report = {
            ...
            "meta": {
                ...
                "include_umap_snapshot": include_umap_snapshot,
            },
            "query": {
                "query_id": query_id,
                "index_id": int(index_id) if index_id else task.index_id,
                "payload": query_payload,
                "status": "done" if query_payload else None,
            },
            "dataset": {
                "id": dataset_id,
                ...
                "preprocess_status": getattr(dataset, "preprocess_status", None),
            },
            "indexes": [
                {
                    "id": item.id,
        ...
        if include_umap_snapshot:
            report["umap_snapshot"] = {
                "highlight_points": query_payload.get("highlight_points"),
                "query_cell_id": query_payload.get("cell_id"),
                "neighbors": query_payload.get("results"),
            }

        report_dir = ensure_dir(settings.report_path)
        stem = f"diagnostic_{dataset_id}_{task_id}"
        json_path = report_dir / f"{stem}.json"
        pdf_path = report_dir / f"{stem}.pdf"
        json_path.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
        _write_pdf_report(report, pdf_path)

        task.result_path = str(json_path)
        task.progress = 100
        task.status = "done"
...
def _build_query_payload(db: Session, payload: dict, query_id: str | None) -> dict:
    query_snapshot = payload.get("query_snapshot") or {}
    query_payload = {k: v for k, v in payload.items() if k != "query_snapshot"}
    if query_snapshot:
        query_payload.update(
            {
                "results": query_snapshot.get("results", []),
    ...
    if query_id:
        ...
        if query_task and query_task.request_payload:
            query_payload.update(query_task.request_payload)
...
def _add_summary_page(pdf, plt, report: dict, font_path: str | None) -> None:
    ...
    query_rows = [
        ...
        ("Query Cell", payload.get("cell_id") or payload.get("query_cell_id")),
...
def _add_embedding_page(pdf, plt, report: dict, font_path: str | None) -> None:
    ...
    ax.text(
        ...
        "A compact cluster indicates that nearest-neighbor search is consistent with the visualization space.",
        color="#334155",
...
def _add_index_page(pdf, plt, report: dict, font_path: str | None) -> None:
    ...
    ax.text(
        0.07,
        0.17,
        "Generated by ANN Retrieval System. Runtime artifacts include JSON/PDF reports, exported result files, "
